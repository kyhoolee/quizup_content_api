...
public class UserPlayStatsProcess {
	...
	public void updatePlayStats(MatchResult match) {
		MatchBaseInfo matchBase = this.matchProcess.getMatchBaseInfo(match.getMatchId());
		...
		this.incrByMatchLog(firstId, matchProcess.getFirstMatchLog(match.getMatchId()).getQuesLog());
		this.incrByMatchLog(secondId, matchProcess.getSecondMatchLog(match.getMatchId()).getQuesLog());
	}
	
	public void incrByMatchLog(String userId, List<MatchQuestionLog> log) {
		int addPoint = 0;
		int addTime = 0;
		
		for(MatchQuestionLog quesLog : log) {
			addPoint += quesLog.getPoint();
			addTime += (20 - quesLog.getPoint());
		}
