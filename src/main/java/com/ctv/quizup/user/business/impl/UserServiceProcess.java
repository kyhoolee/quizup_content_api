...
public class UserServiceProcess implements UserBaseInterface{ 
	...
	public static Logger logger = LoggerUtil.getDailyLogger("UserServiceProcess" + "_error");
	...
	public UserBaseInfo getUser(String userId) {
		...
		try {
			UserResponse response = mapper.readValue(userData, UserResponse.class);
			user = new UserBaseInfo(
					userId, 
					response.user.name,
					response.user.avatarUrl,
					response.user.coverUrl
					);
		} catch (Exception e) {
			logger.error(e);
